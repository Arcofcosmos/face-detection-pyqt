...
from PyQt5.QtCore import Qt, QBasicTimer, QTimer, QDateTime
from PyQt5.QtGui import QFont
from PyQt5 import QtGui
from PyQt5.Qt import QThread

from qwidget import Ui_widget
...
class train_thread(QThread):
    ...
    def run(self):
        ...
        #self.pushButton_2.setText('开始训练')
...
class Example(QWidget, Ui_widget):
    ...
    def __init__(self):
        ...
        self.showTimer = QTimer()        #显示时间
        self.cap = cv2.VideoCapture(0, cv2.CAP_DSHOW)      #视频流       #视频流
        self.CAM_NUM = 0                    #为0时表示视频流来自笔记本内置摄像头
        self.trainThread = train_thread()
    ...
    def initUI(self):
        ...
        self.showTimer.timeout.connect(self.show_current_date_time)
        self.showTimer.start(1000)
        ##设置控件坐标
        #self.lay_widget()
        self.pushButton.clicked.connect(self.onClick)
        self.pushButton_2.clicked.connect(self.train_click)
        self.pushButton_3.clicked.connect(self.button_clicked_detection)
        self.pushButton_4.clicked.connect(self.quit_job)
    ...
    def show_current_date_time(self):
        self.dateLabel.setText(QDateTime.currentDateTime().toString("yyyy/MM/dd"))
        self.timeLabel.setText(QDateTime.currentDateTime().toString("hh:mm:ss"))
    ...
    def onClick(self):
        ...
        #输入名字作为保存文件夹名
        new_user_name = self.InputEdit.text()
    ...
    def CatchPICFromVideo(self):
        ...
        while self.cap.isOpened():
            ...
            self.cameraLabel.setPixmap(QtGui.QPixmap.fromImage(showImage))  #往显示视频的Label里 显示QImage
    ...
    def button_open_camera_det(self):
        if self.timer_camera2.isActive() == False:   #若定时器未启动
            ...
            if flag == False:       #flag表示open()成不成功
                ...
                self.pushButton_3.setText('关闭相机')
        else:
            self.timer_camera2.stop()  #关闭定时器
            self.cap.release()        #释放视频流
            self.cameraLabel.clear()  #清空视频显示区域
            self.pushButton_3.setText('开始识别')
