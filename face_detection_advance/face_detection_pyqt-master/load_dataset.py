...
def read_path(path_name):
    for dir_item in os.listdir(path_name):
        ...
        if os.path.isdir(full_path):    #如果读取到的还是个目录而不是文件则继续向前读取
            read_path(full_path)
        else:
            #判断是人脸照片
            if dir_item.endswith('.jpg'):       #判断文件名是否已'.jpg'结尾，是返回true
                image = cv2.imread(full_path)        #处理图片
                image = resize_image(image)

                images.append(image)           #处理完毕将图片和名字放进列表
...
if __name__ == '__main__':
    if len(sys.argv) != 1:
        ...
        images,labels = load_dataset('D:\\user\\project_developing\\python\\python_project\\face_detection\\data')
        #print(labels)
